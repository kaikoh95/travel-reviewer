...
<script>
  ...
  export default {
    ...
    methods: {
      getOne: function () {
        this.$http.get('http://API_URL:4941/api/v1/users/'+ localStorage.otherUserId, {
      ...
      getImage() {
        this.$http.get('http://API_URL:4941/api/v1/users/'+ localStorage.otherUserId + '/photo')
          .then(function (response) {
            if (response.data !== null && response.data !== undefined && response.data !== "") {
              this.imageView = "http://API_URL:4941/api/v1/users/" + localStorage.otherUserId + "/photo";
            } else {
