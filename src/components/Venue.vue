...
<script>
  ...
  export default {
    ...
    methods: {
      getOne: function () {
        this.$http.get(globalStore.API_URL + '/venues/' + localStorage.venueId)
      ...
      handleUpload() {
        ...
        for (let i=0; i<this.uploads.length; i++) {
          ...
          this.$http.post(globalStore.API_URL + '/venues/' + localStorage.venueId + '/photos',
            data, {
              headers: {
      ...
      getImage(photo) {
        return globalStore.API_URL + '/venues/' + localStorage.venueId + '/photos/' + photo.photoFilename;
      },
      getPrimary() {
        if (this.photos.length > 0) {
          for (let i=0; i<this.photos.length; i++) {
            if (this.photos[i].isPrimary === true) {
              return globalStore.API_URL + '/venues/' + localStorage.venueId + '/photos/' + this.photos[i].photoFilename;
            }
          }
        } else {
      ...
      setPrimary(photo) {
        this.$http.post(
          globalStore.API_URL + '/venues/' + localStorage.venueId + '/photos/' + photo.photoFilename + '/setPrimary',
          "",
      ...
      deletePhoto(photo) {
        this.$http.delete(
          globalStore.API_URL + '/venues/' + localStorage.venueId + '/photos/' + photo.photoFilename,
          {headers: {'X-Authorization': this.$cookies.get("AuthToken")}
          })
          .then(function() {
      ...
      handleSubmit(name) {
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            axios.patch(globalStore.API_URL + '/venues' + localStorage.venueId, JSON.stringify(data),
              {
